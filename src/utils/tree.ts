 ...
 */
export const extractPathList = (tree: any[], attrName = "uniqueId"): any => {
  if (!Array.isArray(tree)) {
    console.warn("tree must be an array");
    return [];
  }
  if (!tree || tree.length === 0) return [];
  let expandedPaths: Array<number | string> = [];
  for (const node of tree) {
    const hasChildren = node.children && node.children.length > 0;
    if (hasChildren) {
      expandedPaths = expandedPaths.concat(
        extractPathList(node.children, attrName)
      );
      // console.log(extractPathList(node.children, attrName));
    }
    expandedPaths.push(node[attrName]);
  }
