...
import { uptConfigApi, getConfigApi } from "@/api/ci/ci-pipeline";
...
type StageTab = { name: string; sort: number };
type EnvTab = Partial<{
  id: number;
  name: string;
  sort: number;
...
function handleEnvSelectConfirm(envData: EnvModel) {
  ...
  newEnvTab.stages = [{ name: "未命名", sort: 1 }];
...
function getRequestData() {
  return state.envTabs.map(envTab => {
    return {
      id: envTab.id,
      stages: envTab.stages.map(stageTab => _.pick(stageTab, ["name"]))
    };
  });
}
